...
class TestPeopleActions(unittest.TestCase):
    ...
    def test_player_bet_errors_on_string(self):
        self.assertEqual(m2.player_bet(self.player, self.dealer, self.deck, "hi"), "Invalid bet. Bet must be an integer")

    def test_player_cant_bet_more_than_stack(self):
        self.assertEqual(m2.player_bet(self.player, self.dealer, self.deck, m2.player_choice('stack', self.player, self.dealer, self.deck) + 300), "Can't bet more than your current stack")
    ...
    def test_dealer_shows_one_card_before_stand(self):
        m2.player_bet(self.player, self.dealer, self.deck, 500)
        value = m2.show_hand(self.dealer)
        self.assertIn("?", value)
